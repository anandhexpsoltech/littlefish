...
class WpdiscuzWalker extends Walker_Comment {
    ...
    public function start_el(&$output, $comment, $depth = 0, $args = array(), $id = 0) {
        ...
        $output .= '<div class="wc-comment-author ' . $uNameClasses . '"><img src="'. get_field('user_avatar', 'user_' . $comment->user_id) . '"/></div>';
        ...
        $output .= '</div>';
        $output .= apply_filters('wpdiscuz_comment_text', '<div class="comment-wrap"><p class="author" style="color: '. get_field('user_color', 'user_' . $comment->user_id) . '">' . $authorName . $afterCommentAuthorName . '<span class="wc-comment-date">' . $posted_date . '</span></p><div class="wc-comment-text">' . $commentContent . '</div>', $comment, $args);
        ...
        if(get_field('field_594555', $comment) == 'Yes') {
            $featured = 'No';
        } else{
            $featured = 'Yes';
        }
        $output .= '<span class="btn-featured ' . get_field('field_594555', $comment) . '"  data-comment="comment_' . $comment->comment_ID . '" data-value="' . $featured . '"></span>';
        ...
        if (!$this->optionsSerialized->votingButtonsShowHide) {
            if ($this->optionsSerialized->votingButtonsStyle) {
                ...
                $output = apply_filters('wpdiscuz_voters', $output, $uniqueId, $comment, $user, $current_user);
                $output .= '<span class="wc-vote-link ' . user_voted($comment->comment_ID) . ' wc-up ' . $voteClass . '">';
                $voteFaUpImg = '<i class="fa fa-thumbs-up fa-flip-horizontal wc-vote-img-up"></i><span>' . $voteUp . '</span>';
                $output .= apply_filters('wpdiscuz_vote_up_icon', $voteFaUpImg, $comment, $current_user);
                $output .= '</span>';
                $output .= '<span class="wc-vote-result ' . user_voted($comment->comment_ID) . '">' . intval($voteCount) . '</span>';
