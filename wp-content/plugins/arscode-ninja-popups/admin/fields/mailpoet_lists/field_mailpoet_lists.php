...
class SNP_NHP_Options_mailpoet_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_mailpoet_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button mailpoet_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
		    'nhp-opts-field-mailpoet_lists-js', SNP_NHP_OPTIONS_URL . 'fields/mailpoet_lists/field_mailpoet_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}