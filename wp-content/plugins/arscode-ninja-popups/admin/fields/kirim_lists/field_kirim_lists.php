...
class SNP_NHP_Options_kirim_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_kirim_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button kirim_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
		    'nhp-opts-field-kirim_lists-js', SNP_NHP_OPTIONS_URL . 'fields/kirim_lists/field_kirim_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}