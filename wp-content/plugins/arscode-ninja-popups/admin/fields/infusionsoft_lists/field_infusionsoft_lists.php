...
class SNP_NHP_Options_infusionsoft_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_infusionsoft_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button inf_lists_gl" name="" value="Grab Groups List" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-infusionsoft_lists-js', SNP_NHP_OPTIONS_URL . 'fields/infusionsoft_lists/field_infusionsoft_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}