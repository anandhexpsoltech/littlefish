...
class SNP_NHP_Options_mautic_stage extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_mautic_stage();
		...
		if(!isset($this->field['meta']) || !$this->field['meta']) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button mautic_stage_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
				'nhp-opts-field-mautic_stage-js', SNP_NHP_OPTIONS_URL . 'fields/mautic_stage/field_mautic_owner.js', array('jquery'), time(), true
		);
	}
}