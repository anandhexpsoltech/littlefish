...
class SNP_NHP_Options_moosend_lists extends SNP_NHP_Options
{
    ...
    public function render()
    {
        ...
        $this->field['options'] = snp_ml_get_moosend_lists();
        ...
        if (!isset($this->field['meta'])) {
            echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button moosend_lists_gl" name="" value="Grab Lists" />';
        }
    ...
    {
        wp_enqueue_script(
            'nhp-opts-field-moosend_lists-js', SNP_NHP_OPTIONS_URL . 'fields/moosend_lists/field_moosend_lists.js', array('jquery', 'farbtastic'), time(), true
        );
    }
}