...
class SNP_NHP_Options_getresponse_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_gr_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button getresponse_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-getresponse_lists-js', SNP_NHP_OPTIONS_URL . 'fields/getresponse_lists/field_getresponse_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}