...
class SNP_NHP_Options_icontact_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_ic_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button icontact_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-icontact_lists-js', SNP_NHP_OPTIONS_URL . 'fields/icontact_lists/field_icontact_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}