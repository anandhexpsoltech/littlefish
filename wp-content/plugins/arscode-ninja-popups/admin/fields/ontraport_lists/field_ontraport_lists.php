...
class SNP_NHP_Options_ontraport_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_ontraport_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button ontraport_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-ontraport_lists-js', SNP_NHP_OPTIONS_URL . 'fields/ontraport_lists/field_ontraport_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}