...
class SNP_NHP_Options_mailup_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_mailup_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button mailup_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-mailup_lists-js', SNP_NHP_OPTIONS_URL . 'fields/mailup_lists/field_mailup_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}