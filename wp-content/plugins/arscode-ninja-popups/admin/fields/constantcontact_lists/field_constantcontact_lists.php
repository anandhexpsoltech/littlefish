...
class SNP_NHP_Options_constantcontact_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_cc_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button constantcontact_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-constantcontact_lists-js', SNP_NHP_OPTIONS_URL . 'fields/constantcontact_lists/field_constantcontact_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}