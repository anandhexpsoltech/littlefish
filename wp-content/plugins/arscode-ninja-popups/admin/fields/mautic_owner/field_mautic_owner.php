...
class SNP_NHP_Options_mautic_owner extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_mautic_owner();
		...
		if (!isset($this->field['meta']) || !$this->field['meta']) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button mautic_owner_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-mautic_owner-js', SNP_NHP_OPTIONS_URL . 'fields/mautic_owner/field_mautic_owner.js', array('jquery'), time(), true
		);
	}
}