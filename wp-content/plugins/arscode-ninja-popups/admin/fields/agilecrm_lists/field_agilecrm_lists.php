...
class SNP_NHP_Options_agilecrm_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options']=snp_ml_get_agilecrm_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button agilecrm_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-agilecrm_lists-js', SNP_NHP_OPTIONS_URL . 'fields/agilecrm_lists/field_agilecrm_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}