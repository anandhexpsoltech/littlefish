...
class SNP_NHP_Options_aweber_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options']=snp_ml_get_aw_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button aweber_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-aweber_lists-js', SNP_NHP_OPTIONS_URL . 'fields/aweber_lists/field_aweber_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}