...
class SNP_NHP_Options_newsletter_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_wy_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button newsletter_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-newsletter_lists-js', SNP_NHP_OPTIONS_URL . 'fields/newsletter_lists/field_newsletter_lists.js', array('jquery'), time(), true
		);
	}
}