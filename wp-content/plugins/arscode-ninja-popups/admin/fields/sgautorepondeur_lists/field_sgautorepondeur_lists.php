...
class SNP_NHP_Options_sgautorepondeur_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_sgautorepondeur_lists();
		...
		if(!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button sgautorepondeur_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
		    'nhp-opts-field-sgautorepondeur_lists-js', SNP_NHP_OPTIONS_URL . 'fields/sgautorepondeur_lists/field_sgautorepondeur_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}