...
class SNP_NHP_Options_klayvio_lists extends SNP_NHP_Options
{
    ...
    public function render()
    {
        ...
        $this->field['options'] = snp_ml_get_klayvio_lists();
        ...
        if (!isset($this->field['meta'])) {
            echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button klayvio_lists_gl" name="" value="Grab Lists" />';
        }
    ...
    {
        wp_enqueue_script(
            'nhp-opts-field-klayvio_lists-js', SNP_NHP_OPTIONS_URL . 'fields/klayvio_lists/field_klayvio_lists.js', array('jquery', 'farbtastic'), time(), true
        );
    }
}