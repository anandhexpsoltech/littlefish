...
class SNP_NHP_Options_sendreach_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_sendreach_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button sendreach_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-sendreach_lists-js', SNP_NHP_OPTIONS_URL . 'fields/sendreach_lists/field_sendreach_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}