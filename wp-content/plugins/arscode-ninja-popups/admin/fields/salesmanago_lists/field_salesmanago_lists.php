...
class SNP_NHP_Options_salesmanago_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options']=snp_ml_get_salesmanago_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button salesmanago_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-salesmanago_lists-js', SNP_NHP_OPTIONS_URL . 'fields/salesmanago_lists/field_salesmanago_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}