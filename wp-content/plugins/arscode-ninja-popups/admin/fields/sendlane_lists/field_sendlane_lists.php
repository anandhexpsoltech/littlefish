...
class SNP_NHP_Options_sendlane_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_sendlane_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button sendlane_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-sendlane_lists-js', SNP_NHP_OPTIONS_URL . 'fields/sendlane_lists/field_sendlane_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}