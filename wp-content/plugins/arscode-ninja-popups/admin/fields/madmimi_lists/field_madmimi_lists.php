...
class SNP_NHP_Options_madmimi_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_madm_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button madmimi_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-madmimi_lists-js', SNP_NHP_OPTIONS_URL . 'fields/madmimi_lists/field_madmimi_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}