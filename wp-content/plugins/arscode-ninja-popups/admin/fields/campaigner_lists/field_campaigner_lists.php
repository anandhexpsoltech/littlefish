...
class SNP_NHP_Options_campaigner_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_campaigner_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button campaigner_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
		    'nhp-opts-field-campaigner_lists-js', SNP_NHP_OPTIONS_URL . 'fields/campaigner_lists/field_campaigner_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}