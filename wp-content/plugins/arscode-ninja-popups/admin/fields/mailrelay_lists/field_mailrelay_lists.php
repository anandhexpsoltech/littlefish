...
class SNP_NHP_Options_mailrelay_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_mailrelay_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button mailrelay_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-mailrelay_lists-js', SNP_NHP_OPTIONS_URL . 'fields/mailrelay_lists/field_mailrelay_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}