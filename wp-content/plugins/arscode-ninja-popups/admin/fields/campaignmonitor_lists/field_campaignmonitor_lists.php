...
class SNP_NHP_Options_campaignmonitor_lists extends SNP_NHP_Options
{
	...
	public function render()
	{
		...
		$this->field['options'] = snp_ml_get_cm_lists();
		...
		if (!isset($this->field['meta'])) {
		    echo '<input type="button" rel-id="' . $this->field['id'] . '" class="button campaignmonitor_lists_gl" name="" value="Grab Lists" />';
		}
	...
	{
		wp_enqueue_script(
			'nhp-opts-field-campaignmonitor_lists-js', SNP_NHP_OPTIONS_URL . 'fields/campaignmonitor_lists/field_campaignmonitor_lists.js', array('jquery', 'farbtastic'), time(), true
		);
	}
}